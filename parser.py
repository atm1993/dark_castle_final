...
class Parse:
  ...
  def parse_user_input(self, input_string):
    ...
    if len(word_list)>=1 and cmd is None:
      if self.match_noun(word_list, self.rooms) is not None or word_list[0] in self.direction:
        cmd = "go"
      else:
        return None, None, None, None, None
    elif len(word_list) > 1 and cmd is None:
      return None, None, None, None, None
    ...
    if cmd == "go":
      ...
      if cardinal_direction is None and room is None and not word_list:
